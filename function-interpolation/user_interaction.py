from math import sin, sqrt
from matplotlib import pyplot as plt


def print_finite_differences_table(delta_y):
    n = delta_y.shape[0]
    print("Таблица конечных разностей:")
    for i in range(n):
        row = [f"{delta_y[i, j]:.4f}" if i + j < n else "" for j in range(n)]
        print("\t".join(row))


def draw_plot(a, b, func, name, dx=0.001):
    xs, ys = [], []
    a -= dx
    b += dx
    x = a
    while x <= b:
        xs.append(x)
        ys.append(func(x))
        x += dx
    plt.plot(xs, ys, 'g', label=name)


def read_data_from_file(filename):
    try:
        with open(filename, 'r') as file:
            xs = []
            ys = []
            x_read = False
            for line in file:
                if not x_read:
                    x_read = True
                    x = float(line.strip())
                else:
                    point = line.strip().split()
                    if len(point) == 2:
                        xs.append(float(point[0]))
                        ys.append(float(point[1]))

        return x, xs, ys, None
    except IOError as err:
        return None, None, None, "! Невозможно прочитать файл {0}: {1}".format(filename, err)


def read_data_from_terminal():
    x = float(input("Введите точку интерполяции: "))
    str = ' '
    xs = []
    ys = []
    print("Введите Enter, чтобы закончить ввод.")
    print("Введите узлы интерполяции:")
    while str:
        str = input()
        point = str.strip().split()
        if len(point) == 2:
            xs.append(float(point[0]))
            ys.append(float(point[1]))
        else:
            if str != '':
                print("! Некорректный ввод. Введенная точка не будет использована.")
    return x, xs, ys


def read_data_from_function():
    print('Функции: ')
    print('1. 2*x^2 - 5*x')
    print('2. x^5')
    print('3. sin(x)')
    print('4. sqrt(x)')

    while True:
        input_func = int(input('Выберите функцию [1/2/3/4]: '))
        if input_func == 1:
            f = lambda x: 2*x**2 - 5*x
            break
        elif input_func == 2:
            f = lambda x: x**5
            break
        elif input_func == 3:
            f = lambda x: sin(x)
            break
        elif input_func == 4:
            f = lambda x: sqrt(x)
            break
        else:
            print("! Некорректный ввод. Попробуйте еще раз\n")

    n = int(input("Введите число узлов: "))
    x0 = float(input('Введите x0: '))
    xn = float(input('Введите xn: '))

    h = (xn - x0) / (n - 1)
    xs = [x0 + h * i for i in range(n)]
    ys = list(map(f, xs))

    x = float(input('Введите точку интерполяции: '))
    return x, xs, ys


def get_params():
    while True:
        while True:
            print("Введите:\n  - 'f' для ввода из файла;\n  - 't' для ввода с терминала;\n  - 'fu' для задания функции.")
            option = input("Ваш ввод: ")
            if option == 'f':
                while True:
                    filename = input("Введите имя файла: ")
                    x, xs, ys, error = read_data_from_file(filename)
                    if error is not None:
                        print(error)
                        continue
                    else:
                        break
                n = len(xs)
                break
            elif option == 't':
                x, xs, ys = read_data_from_terminal()
                n = len(xs)
                break
            elif option == 'fu':
                x, xs, ys = read_data_from_function()
                n = len(xs)
                break
            else:
                print("! Некорректный ввод. Попробуйте еще раз\n")

        if len(set(xs)) != len(xs):
            print('! Узлы интерполции не должны совпадать. Введите еще раз.')
        elif xs != sorted(xs):
            print('! Узлы интерполяции должны быть отсортированы. Введите еще раз.')
        else:
            break

    return xs, ys, x, n
